...
for i = 1:length(t)
    ...
    W1 = sol(7:27)';  % Critic Weights
    W2 = sol(28:48)'; % Actor Weights
    W3 = sol(49:69)'; % Disturbance Weights
    ...
    R11 = 2*R22;
    R12 = 2*R21;
    u3 = -(1/2) * inv(R) * gm' * grad_phi1' * W2;
    d4 = -(1/2) * inv(R) * km' * grad_phi1' * W3;
...
% Subplot 1: Position
subplot(2, 1, 1);
plot(t(1:end), u3_sol(1, 1:end), '.r', 'LineWidth', 1.5); 
hold on; % Black dashed line
plot(t(1:end), u3_sol(2, 1:end), ':k', 'LineWidth', 1.5);
hold on
plot(t(1:end), u3_sol(3, 1:end), '-b', 'LineWidth', 1.5);          % Black solid line
xlim([0, 1200])
...
% Subplot 2: Velocity
subplot(2, 1, 2);
plot(t(1:end), d4_sol(1, 1:end), '.r', 'LineWidth', 1.5); 
hold on; % Black dashed line
plot(t(1:end), d4_sol(2, 1:end), ':k', 'LineWidth', 1.5); 

hold on;
plot(t(1:end), d4_sol(3, 1:end), '-b', 'LineWidth', 1.5);          % Black solid line
xlabel('Time (sec)', 'FontSize', 12, 'FontName', 'Times New Roman');
ylabel('Disturbance Control Input (m/s^2)', 'FontSize', 12, 'FontName', 'Times New Roman');
legend({'$Dx$', '$Dy$', '$Dz$'}, 'Interpreter', 'latex', 'FontSize', 10, 'Location', 'northeast', 'Box', 'off');
box on; % Enable thick outline
set(gca, 'FontName', 'Times New Roman', 'FontSize', 12, 'LineWidth', 1); % Thicker bounding box
xlim([0, 1200])
